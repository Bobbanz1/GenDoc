...
class App(CTk.CTk):
    def __init__(self, *args, **kwargs):
        ...
        self.label = CTk.CTkLabel(self, text="Document Creator v1.1", font=CTk.CTkFont(size=20, weight="bold"))
    ...
    def open_html_window(self):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            self.toplevel_window = GUI.HTMLWindow(self)
        else:
            self.toplevel_window.focus()

    def open_markdown_window(self):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            self.toplevel_window = GUI.MarkdownWindow(self)
        else:
            self.toplevel_window.focus()

    def open_plain_window(self):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            self.toplevel_window = GUI.TextWindow(self)
        else:
            self.toplevel_window.focus()
